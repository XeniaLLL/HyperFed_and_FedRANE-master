...
def generate_fmnist(dir_path, num_clients, num_classes, niid, balance, partition):
    ...
    dataset_image, dataset_label = fmnist_ndarray(os.path.join(dir_path, 'rawdata'))
...
def main_legacy():
    ...
    num_clients = 20
    num_classes = 10
    dir_path = "/home/lxt/code/datasets/fmnist/"
...
def fmnist_ndarray(root: str):
    return vision_dataset_ndarray(
        root=root,
        dataset_type=torchvision.datasets.FashionMNIST,
...
def main():
    random.seed(1)
    np.random.seed(1)
    data, labels = fmnist_ndarray(os.path.join(args.dir_path, 'rawdata'))
