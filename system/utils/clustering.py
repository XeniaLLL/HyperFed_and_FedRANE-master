...
def gen_data(iid, dataset_type, data_root_dir, transforms_dict, num_users, cluster, dataset_split=''):
    ...
    _, image_datasets, dataset_sizes, class_names = \
        load_dataset(dataset_type, data_root_dir, transforms_dict, batch_size=8, shuffle_flag=False,
...
def partition_clusters(clustering_matrix, args, nr_clusters=5, method='complete', metric='euclidean', plotting=False):
    ...
    cluster_memberships = sch.fcluster(Y, t=nr_clusters,
                                       criterion='maxclust')  # ith element in this array is the cluster for i
    ...
    if plotting:
        fig.savefig(
            f'{args.results_root_dir}/clust_{args.clustering_method}_nr_users-{args.num_users}_nr_of_partition_clusters_{nr_clusters}_method_{method}_reconstructed.png')
    ...
    if plotting:
        fig.savefig(
            f'{args.results_root_dir}/clust_{args.clustering_method}_nr_users-{args.num_users}_nr_of_partition_clusters_{nr_clusters}_method_{method}_filtered.png')
    ...
    for i, cluster_members_a in cluster_user_dict.items():
        for j, cluster_members_b in cluster_user_dict.items():
            assert np.all(cluster_members_a != cluster_members_b) or set(cluster_members_a).intersection(
                set(cluster_members_b)) != {}, f'clusters {i} and {j} are not disjoint'
...
if __name__ == '__main__':
    ...
    if args.clustering_method == 'umap':
        clustering_matrix0, clustering_matrix0_soft, centers = clustering_umap(args.num_users, dict_users,
                                                                               dataset_train, args)
    #
    ...
    plt.savefig(
        f'{args.results_root_dir}/Clustering/clustMat_perfect_nrclust_nrusers-{args.num_users}-{args.nr_of_embedding_clusters}_from-{args.pre_trained_dataset}_to-{args.target_dataset}.jpg')

    plt.figure(2)
    plt.imshow(clustering_matrix0, cmap=plt.cm.viridis)
    plt.savefig(
        f'{args.results_root_dir}/Clustering/clustMat_{args.clustering_method}_nrusers-{args.num_users}_nrclust-{args.nr_of_embedding_clusters}_from-{args.pre_trained_dataset}_to-{args.target_dataset}.jpg')

    plt.figure(3)
    plt.imshow(-1 * clustering_matrix0_soft, cmap=plt.cm.viridis)
    plt.savefig(
        f'{args.results_root_dir}/Clustering/softClustMat_{args.clustering_method}_nrusers-{args.num_users}_nrclust-{args.nr_of_embedding_clusters}_from-{args.pre_trained_dataset}_to-{args.target_dataset}.jpg')
    ...
    colors = ['r'] * nr_of_centers + ['b'] * nr_of_centers + ['g'] * nr_of_centers + ['k'] * nr_of_centers + [
        'y'] * nr_of_centers
    ...
    ax.set_zticklabels([])
    plt.savefig(
        f'{args.results_root_dir}/Clustering/ecnoder_pca_plot3d_{args.clustering_method}_nrusers-{args.num_users}_nrclust-{args.nr_of_embedding_clusters}_from-{args.pre_trained_dataset}_to-{args.target_dataset}.jpg')
    ...
    if args.clustering_method not in ['umap_central', 'umap']:
        ...
        for i in range(args.nr_of_embedding_clusters):
            plt.scatter(embedding_matrix[i * nr_of_centers:(i + 1) * nr_of_centers, 0],
                        embedding_matrix[i * nr_of_centers:(i + 1) * nr_of_centers:, 1], color=next(colors))
        plt.savefig(
            f'{args.results_root_dir}/Clustering/embeddingMat_{args.clustering_method}_nrusers-{args.num_users}_nrclust-{args.nr_of_embedding_clusters}_from-{args.pre_trained_dataset}_to-{args.target_dataset}.jpg')
    elif args.clustering_method == 'umap_central':
        plt.figure(5)
        nr_of_centers = len(c_dict[0]) * cluster_length
        colors = itertools.cycle(
            ["r"] * nr_of_centers + ["b"] * nr_of_centers + ["g"] * nr_of_centers + ["k"] * nr_of_centers + [
                "y"] * nr_of_centers)
        ...
        # c_dict
        plt.savefig(
            f'{args.results_root_dir}/Clustering/umap_embeding_{args.clustering_method}_nrusers-{args.num_users}_nrclust-{args.nr_of_embedding_clusters}_from-{args.pre_trained_dataset}_to-{args.target_dataset}.jpg')
    plt.show()
