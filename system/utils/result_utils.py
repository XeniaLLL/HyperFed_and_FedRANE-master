...
def average_data(algorithm="", dataset="", goal="", times=10, length=800, suffix=""):
    test_acc = get_all_results_for_one_algo(
        algorithm, dataset, goal, times, int(length), suffix)
...
def get_all_results_for_one_algo(algorithm="", dataset="", goal="", times=10, length=800, suffix=""):
    test_acc = np.zeros((times, length))
    algorithms_list = [algorithm] * times
    for i in range(times):
        file_name = dataset + "_" + \
            algorithms_list[i] + "_" + goal + "_" + str(i)+suffix
        test_acc[i, :] = np.array(
