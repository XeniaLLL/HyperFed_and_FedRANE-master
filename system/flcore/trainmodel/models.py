...
class LocalGModel(nn.Module):
    def __init__(self, base, graph_generator, gnn, predictor, alpha=0.9):
        super(LocalGModel, self).__init__()
        self.base = base
    ...
    def forward(self, x):
        ...
        preds, Z1_aug = self.model.gnn(out, A)  # careful 把preds 利用起来
        # Z1_edge_attr, Z1_edge_index, Z1_aug = self.graph_generator.get_graph(out)
        # preds, Z1_aug = self.gnn(Z1_aug, Z1_edge_index) # careful 把preds 利用起来
        # preds, Z1_aug = self.gnn(Z1_aug, Z1_edge_index, Z1_edge_attr, self.output_train_gnn) # careful 把preds 利用起来
        out = self.alpha* out +(1-self.alpha)*Z1_aug[-1]  # careful
        out = self.predictor(out)
        return out


class LocalBayesModel(LocalModel):
    def __init__(self, base, predictor, mu_estimator, kappa_estimator, use_dedimension=False):
        super(LocalBayesModel, self).__init__(base, predictor)
        self.mu_estimator = mu_estimator
        self.kappa_estimator = kappa_estimator
        if use_dedimension:  # de-dimension for resnet50
            self.de_dimension = nn.Sequential(
                nn.BatchNorm2d(512),
                nn.Dropout(0.4),
                nn.Flatten(),
                nn.Linear(512 * 14 * 14, 512),
            )
        self.use_dedimension = use_dedimension

    def forward(self, x):
        out = self.base(x)
        if self.use_dedimension:
            out = self.de_dimension(out)
        mu = self.mu_estimator(out)
        kappa = self.kappa_estimator(out)
